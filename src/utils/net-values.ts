import { EastMoneyService, NUM_ITEM_PER_PAGE } from "../services/eastmoney/eastmoney-service";
import { NetValue } from "../analyze/analyze";

export function getNetValues({ eastMoneyService, numDays, fundId }: {
  ...
  const promises = pageIdx.reverse().map(idx => eastMoneyService.getNetValues({ id: fundId, pageNum: idx }));
