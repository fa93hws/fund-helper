...
impl EastMoneyService<'_> {
    ...
    pub fn fetch_list(&self) -> FundList {
        let http_result = self.http_service.get(FETCH_LIST_URL);
...
mod tests {
    ...
    fn test_fetch_success() {
        ...
        let model = east_money_service.fetch_value(FUND_ID, 3);
    ...
    fn test_fetch_fail() {
        struct FakeHttpService {}
        impl CanGetHTTP for FakeHttpService {
            fn get(&self, _: &str) -> Result<String, HttpError> {
        ...
        let http_service = FakeHttpService {};
        let east_money_service = EastMoneyService::new(&http_service);
        east_money_service.fetch_value(FUND_ID, 2);
    }
}
