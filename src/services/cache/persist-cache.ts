import type { LocalIOService } from '../local-io/local-io';
...
export class PersistCacheService {
  ...
  set({ key, value, version }: { key: string; value: object; version: string }) {
  ...
  private static assertCacheContent(key: string, content: any) {
    if (typeof content.timeStamp !== 'number') {
      return { kind: 'badCache' as const, reason: `unable to parse ${key}, timestamp must be number` }
    }
    if (typeof content.value !== 'object') {
      return { kind: 'badCache' as const, reason: `unable to parse ${key}, value must be object` }
    }
    if (typeof content.version !== 'string') {
      return { kind: 'badCache' as const, reason: `unable to parse ${key}, version must be string` }
    }
    return content as PersistCacheOutput;
  }
  // age: ms
  get({ key, age, version }: { key: string; age: number; version: string }): CacheGetResult {
    ...
    try {
      ...
      const parsedCacheContent = PersistCacheService.assertCacheContent(key, cacheContent);
      ...
      return { kind: 'badCache', reason: `unable to parse ${key}, content: ${content}` };
    }
  }
}